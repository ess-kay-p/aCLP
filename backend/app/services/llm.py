...
def generate_diagram_svg(topic: str, labels: Optional[List[str]] = None) -> Optional[str]:
    ...
    prompt = (
        f"Generate a clean educational SVG diagram for the topic: '{topic}'.\n"
        f"The diagram must clearly label these parts: {label_str}.\n\n"
        "Requirements:\n"
        "- Output ONLY the raw SVG code, nothing else — no markdown, no explanation\n"
        "- viewBox='0 0 600 400', width='600', height='400'\n"
        "- White background rect\n"
        "- Use simple shapes: rect, circle, ellipse, path, line, polygon\n"
        "- Every labeled part must have a visible text label in plain English\n"
        "- Use arrows (lines with arrowhead markers) to connect labels to parts\n"
        "- Font: font-family='Arial, sans-serif', font-size between 12 and 16\n"
        "- Use 2-3 colors max, keep it clean and uncluttered\n"
        "- All elements must be fully inside the viewBox with padding\n"
